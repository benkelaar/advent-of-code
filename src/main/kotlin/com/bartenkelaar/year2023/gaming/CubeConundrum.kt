...
private data class Game(
    val id: Int,
    val handsShown: List<Set<Cubes>>
) {
    fun isPossible() = CubeColour.entries.all { maxShown(it) <= it.available }

    fun power() = CubeColour.entries.productOf { maxShown(it) }

    private fun maxShown(colour: CubeColour) =
        handsShown.maxOf { hand -> hand.find { it.colour == colour }?.amount ?: 0 }
}

class CubeConundrum : Solver() {
    override fun solve(input: List<String>): Pair<Any, Any> {
        val games = input.nonBlank()
